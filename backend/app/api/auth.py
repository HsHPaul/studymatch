...
from app.core.blacklist import is_chat_blocked, is_email_blocked
...
def register(request: Request, payload: RegisterRequest, db: Session = Depends(get_db)):
    if is_email_blocked(payload.email):
        raise HTTPException(status_code=400, detail="Diese E-Mail-Adresse ist nicht erlaubt.")
    if is_chat_blocked(payload.alias):
        raise HTTPException(status_code=400, detail="Dieser Anzeigename ist nicht erlaubt.")
